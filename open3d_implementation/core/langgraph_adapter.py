...
"""
                    ...
                    if comfyui_url:
                        ...
                        try:
                            ...
                            if test_response.status_code == 200:
                                ...
                                print(
                                    f"⚠️ ComfyUI respondeu com status: {test_response.status_code}"
                                )
                        except requests.RequestException as conn_e:
                            print(f"⚠️ Erro ao conectar ComfyUI: {conn_e}")
                            raise RuntimeError("ComfyUI não acessível") from conn_e
                            ...
                            if prompt_id:
                                ...
                                while wait_time < max_wait:
                                    ...
                                    if history_response.status_code != 200:
                                        print(
                                            f"⚠️ Erro ao consultar histórico: {history_response.status_code}"
                                        )
                                        continue
                                    ...
                                    if prompt_id not in history:
                                        print(
                                            f"⏳ Prompt ainda não encontrado no histórico... ({wait_time}s)"
                                        )
                                        continue
                                    ...
                                    if not images:
                                        print(
                                            f"⏳ Aguardando geração... ({wait_time}s)"
                                        )
                                        continue

                                    print("🖼️ Imagem gerada! Baixando...")
                                    image_data = images[0]
                                    image_response = requests.get(
                                        f"{comfyui_url}/view?filename={image_data['filename']}",
                                        timeout=30,
                                    )
                                    ...
                                    if (
                                        ...
                                        print(
                                            ...
                                            f"cena {scene['scene_id']} ({image_size} bytes)"
                                        )
                                        continue

                                    print("⚠️ Arquivo baixado não é uma imagem válida")
                                else:
                                    print(f"⏱️ Timeout após {max_wait}s")
                            else:
                                print("⚠️ Nenhum prompt_id retornado")
                        else:
                            print(
                                f"⚠️ Erro ao enviar workflow: {response.status_code} - {response.text}"
                            )
        ...
        def generate_audio(state: Open3DAgentState) -> Open3DAgentState:
            ...
            for i, scene in enumerate(scenes[:3]):  # Limit to 3 scenes
                            ...
                            if response.status_code == 200:
                                ...
                                if (
                                    ...
                                    print(
                                        ...
                                        f"cena {scene['scene_id']} ({audio_size} bytes)"
                                    )
                                    continue
                                else:
                ...
                except (KeyError, OSError, requests.RequestException) as e:
